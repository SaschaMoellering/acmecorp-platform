...
There are two broad approaches to retry in a Spring AMQP application. The first is broker-side: nack the message, let the broker redeliver it, and rely on dead-letter configuration to cap the attempts. The second is container-side: catch the exception inside the listener container, apply backoff and retry logic there, and only involve the broker once retries are exhausted. This implementation uses the container-side approach. The Spring AMQP listener container is configured with a stateless retry interceptor, which is appropriate here because the listener can be retried directly without relying on a broader surrounding transaction. If the listener participates in a larger transactional context, a stateful retry strategy is typically required to preserve consistency across attempts. The broker does not see individual retry attempts. It only sees the final outcome: either a successful ack, or a rejection without requeue that routes the message to the dead-letter exchange.
...
The dead-letter queue is not a failure. It is a safety net. A system without a dead-letter queue loses messages silently. A system with a dead-letter queue prevents silent loss in well-configured systems. The message is there, waiting, until someone decides what to do with it.
...
RabbitMQ guarantees that a message published to a durable queue will survive a broker restart, provided the message is marked as persistent and the queue is declared as durable. There is still a short window between the broker accepting the message and flushing it to disk where a hard crash can cause loss. Publisher confirms mean the broker has taken responsibility for the message, typically after persistence, but not strictly equivalent to a fully durable commit in all configurations. Without publisher confirms, the producer has no way to know whether a message survived a broker crash between accept and flush.

[SHOW: queue declaration – durable: true, message properties – deliveryMode: PERSISTENT]

RabbitMQ can be configured to achieve at-least-once delivery when the listener container is configured to acknowledge only after successful processing. If the container uses automatic acknowledgement at the point of delivery rather than at the point of successful completion, a consumer crash after delivery but before processing loses the message permanently. The distinction is in the container's acknowledgement mode, not in whether the listener method calls ack explicitly.

Messages in a single queue are delivered in best-effort FIFO under single-consumer conditions. That ordering breaks down in at least three common situations: when a nacked message is requeued ahead of newer messages, when multiple consumers process from the same queue concurrently, or when a message is routed through a dead-letter exchange and re-enters a different queue position. RabbitMQ offers no per-consumer ordering guarantee across any of these cases. If your consumer logic depends on processing messages in a specific order, you need to design for that explicitly, not rely on the broker to enforce it.
...
The idempotency record needs to be stored somewhere durable. Storing it in memory is not sufficien# Episode 12 — Asynchronous Messaging with RabbitMQ
...
The dead-letter queue is not a failure. It is a safety net. A system without a dead-letter queue loses messages silently. A system with a dead-letter prevents silent loss in well-configured systems. The message is there, waiting, until someone decides what to do with it.
...
For local side effects, a pattern of checking for an existing record and persisting the result within the same transaction can make duplicate deliveries harmless. This works because the side effect and the idempotency record can be committed atomically. When the side effect is external — calling a payment provider, triggering a third-party email API — the pattern becomes harder, because the external action and the local record cannot usually be committed atomically. The external call may succeed while the record insert fails, or vice versa. In those cases, the idempotency key needs to be scoped to the external system as well, not just to the local database. It applies to any consumer that has side effects: sending emails, charging payment methods, updating external systems. The side effect should happen at most once per message, and the idempotency record is what enforces that.
...
For local side effects, a pattern of checking for an existing record and persisting the result within the same transaction can make duplicate deliveries harmless. This works because the side effect and the idempotency record can be committed atomically. When the side effect is external — calling a payment provider, triggering a third-party email API — the pattern becomes harder, because the external action and the local record cannot usually be committed atomically. The external call may succeed while the record insert fails, or vice versa. In those cases, the idempotency key needs to be scoped to the external system as well, not just to the local database. In those cases, the idempotency key needs to be scoped to the external system as well, not just to the local database. It applies to any consumer that has side effects: sending emails, charging payment methods, updating external systems. The side effect should happen at most once per message, and the idempotency record is what enforces that.
...
Decouple deliberately. Observe everything.
, because a consumer restart would lose the record and allow duplicate notifications. Storing it in the database alongside the notification record is the right approach. The check and the insert should happen in the same transaction so that a crash between them does not leave the system in an inconsistent state. Note that this pattern makes duplicates harmless, not impossible. The message will still be delivered more than once. The idempotency record is what ensures the second delivery produces no additional side effect.
