...
# Avoid branch checkout failures due to build artifacts / node_modules
if ! git diff --quiet || [[ -n "$(git ls-files -o --exclude-standard)" ]]; then
  echo "Working tree not clean. Stash/clean build artifacts before running:" >&2
  echo "  git clean -fdX   # removes ignored files (after you fix .gitignore)" >&2
  echo "  or: git stash -u" >&2
  exit 1
fi
...
# Optional: run a single branch, e.g. ONLY_BRANCH=java21 bash bench/run-matrix.sh
...
function use_branch_java() {
  ...
  if java_home="$(pick_java_home_sdkman "$version")"; then
    ...
    echo "No JDK found for Java ${version}. Install via SDKMAN or under /usr/lib/jvm." >&2
    exit 1
  fi
...
for branch in "${branches[@]}"; do
  ...
  # loadtest output may contain stderr noise -> capture combined and extract first JSON object
...
PY
  ...
  # Ensure collect.sh runs with PYTHON_BIN available
